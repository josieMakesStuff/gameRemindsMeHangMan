...
export default function AssemblyEndgame() {
    ...
    const keyboardElements = alphabet.split("").map(letter => {
      ...
      return(<button className={className} key={letter} disabled={isGameOver} aria-disabled={guessedLetters.includes(letter)} aria-label={`Letter ${letter}`} onClick={() => addGuessedLetter(letter)}>{letter.toUpperCase()}</button>)
    })
    ...
    return (
        <main>
            ...
            <header>
              ...
              <p>Guess the word within 8 attempts to keep the programming Assembly!</p>
            </header>
            ...
            <section className="sr-only" aria-live="polite" role="status">
              ...
              <p>So far, the current word: {currentWord.split("").map(letter =>
